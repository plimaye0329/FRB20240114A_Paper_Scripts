...
class UBBBurster:
    def __init__(self, file_band1, file_band2, file_band3, file_band4, file_band5, burst_mjd, dm):
        ...
        print('Toa in band 1 is',self.toa1)
        print('Toa in band 2 is',self.toa2)
        print('Toa in band 3 is',self.toa3)
        print('Toa in band 4 is',self.toa4)
        print('Toa in band 5 is',self.toa5)
...
def main(file_band1, file_band2, file_band3, file_band4, file_band5, burst_mjd, dm, bins, output_prefix, combined_output_filename_prefix, n_samples, final_output_png):
    ...
    stitched_waterfall, stitched_f_channels = insert_nan_band(combined_waterfall_1_2, combined_waterfall_3_4_5, combined_f_channels_1_2, combined_f_channels_3_4_5, freq_gap_start, freq_gap_end, freq_gap_bins=800)
    sk = kurtosis(stitched_waterfall, axis=1, fisher=False, bias=False)
    rfi_mask = (sk < 2.5) | (sk > 4.5)
    stitched_waterfall[rfi_mask, :] = np.nanmedian(stitched_waterfall)
   ...
   # Set multiple frequency ranges to NaN: (These are known rfi channels, edit this according to the rfi environment)
    ...
    time_series = np.nanmean(stitched_waterfall[1300:], axis=0)
    ...
    # Plot the results
    fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(8, 12), gridspec_kw={'height_ratios': [1, 3]})
    ax1.plot(time_centered, time_series[start_index:end_index], color='black')
    ax1.set_ylabel('S/N',fontsize=20)
    ax1.grid(False)
    ...
    custom_yticks = [1300, 2000, 3000, 4000, 5000, 6000]
    ax2.set_yticks(custom_yticks,fontsize=18)
    ax2.set_yticklabels([f'{tick:.1f}' for tick in custom_yticks])
    for ypos in [1939.58984375, 4101.328125, 5226.328125]:

        ax2.axhline(y=ypos, color='white', linestyle='--')


    plt.subplots_adjust(hspace=0.1)
    plt.tight_layout()
    print(f"Saving final plot as: {final_output_png}")

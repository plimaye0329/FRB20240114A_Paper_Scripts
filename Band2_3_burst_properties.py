...
def extract_sliced_spectrum(waterfall):
    ...
    return waterfall[:, start_index:end_index]

def combine_waterfalls(waterfalls, f_channels_list, t_res_list):
    combined_waterfall = np.concatenate(waterfalls, axis=0)
    combined_f_channels = np.concatenate(f_channels_list)
    combined_t_res = t_res_list[0]
    return combined_waterfall, combined_f_channels, combined_t_res
...
def process_files(file_band1_ar,file_band2_ar, nchan1, nchan2, output_prefix):
    results_summary = []
    gaussian_mjd_list = []
    print(f"file_band1_ar: {file_band1_ar} (type: {type(file_band1_ar)})")
    print(f"file_band2_ar: {file_band2_ar} (type: {type(file_band2_ar)})")


    pdf_pages = PdfPages(f"{output_prefix}_plots.pdf")
    error_log = open(f"{output_prefix}_error.log", "w")
    #files = [file_band1_ar,file_band2_ar]
    for filename in file_band1_ar, file_band2_ar:
        ...
        waterfall1, f_channels1, t_res1 = load_psrchive(file_band1_ar, nchan1)
        waterfall2, f_channels2, t_res2 = load_psrchive(file_band2_ar, nchan2)
        sliced1 = extract_sliced_spectrum(waterfall1)
        sliced2 = extract_sliced_spectrum(waterfall2)

        #combined_waterfall, combined_f_channels, combined_t_res = combine_waterfalls([sliced1, sliced2], [f_channels1, f_channels2],[t_res1, t_res2])
        combined_waterfall1, combined_f_channels1, combined_t_res1 = combine_waterfalls([sliced1],[f_channels1],[t_res1])
        combined_waterfall2, combined_f_channels2, combined_t_res2 = combine_waterfalls([sliced2],[f_channels2],[t_res2])
        freq_gap_start = combined_f_channels1.max()
        freq_gap_end = combined_f_channels2.min()
        stitched_waterfall, stitched_f_channels = insert_nan_band(combined_waterfall1, combined_waterfall2, combined_f_channels1, combined_f_channels2, freq_gap_start, freq_gap_end, freq_gap_bins=400)
        
        combined_waterfall = stitched_waterfall
        combined_f_channels = stitched_f_channels
        block_size = (1, 1)
        waterfall_reduced = block_reduce(combined_waterfall, block_size=block_size, func=np.mean)
        t_res_reduced = combined_t_res2 * block_size[1]
        f_channels_reduced = combined_f_channels[::block_size[0]]
        ...
        for group_id, indices in enumerate(grouped_indices):
            ...
            l = 1.882e27  # cm
            z = 0.13
            print(t_res_reduced, 'time resolution')
...
# Fluence localized to this group
            if group_max > group_min:
                fluence = (np.sum(time_series[group_min:group_max]) * t_res_reduced) / 1000  # mJy * s = Jy-s
            else:
                fluence = np.nan

# Width = weighted mean sigma
            if group_sigmas:
                burst_width = (rightmost_intercept - leftmost_intercept)*t_res_reduced*1e3 # width in ms
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f1', '--file_band1_ar', type=str, required=True, help='Band1 archive file')
    parser.add_argument('-f2', '--file_band2_ar', type=str, required=True, help='Band2 archive file')
    parser.add_argument('-n1', '--nchan1', type=int, required=True, help='Number of frequency channels to scrunch to')
    parser.add_argument('-n2', '--nchan2', type=int, required=True, help='Number of frequency channels to scrunch to')
    parser.add_argument('-o', '--output_prefix', type=str, required=True, help='Output prefix for results')

    args = parser.parse_args()
    process_files(args.file_band1_ar, args.file_band2_ar, args.nchan1,args.nchan2, args.output_prefix)

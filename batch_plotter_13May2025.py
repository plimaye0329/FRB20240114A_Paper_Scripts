...
def process_files(files, nchan, dm, output_prefix):
    ...
    for filename in files:
        ...
        for group_id, indices in enumerate(grouped_indices):
            ...
            leftmost_intercept = int(np.min([i[0] for i in intercepts]))
            rightmost_intercept = int(np.max([i[1] for i in intercepts]))
            # ✅ Compute burst width using sub-burst intercepts
            burst_width = (rightmost_intercept - leftmost_intercept) * t_res_reduced * 1e3  # in ms
            burst_width_uncertainty = burst_width * 0.1 if not np.isnan(burst_width) else np.nan

    # ✅ Compute fluence for this specific sub-burst
            if 0 <= leftmost_intercept < rightmost_intercept <= len(time_series):
            ...
            num_ticks = 3
            yticks = np.linspace(f_channels_reduced.min(), f_channels_reduced.max(), num=num_ticks)

            fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(17, 20), gridspec_kw={'height_ratios': [1, 3]})
            ax1.plot(zoomed_time*t_res_reduced, zoomed_time_series, color='black', label='Time Series')
            # Mark burst width (intercepts)
            if results["fitted_params"]:
                ax1.plot(zoomed_time*t_res_reduced, multi_gaussian(zoomed_time, *results["fitted_params"]), color='red', label='Gaussian Fit')

            ax1.axvline(x=leftmost_intercept*t_res_reduced, color='blue', linestyle='--', linewidth=1.5)
            ax1.axvline(x=rightmost_intercept*t_res_reduced, color='blue', linestyle='--', linewidth=1.5)

            im = ax2.imshow(waterfall_reduced[:, zoom_start:zoom_end], aspect='auto', cmap='plasma', origin='lower',
                            extent=[time[zoom_start]*t_res_reduced, time[zoom_end]*t_res_reduced, f_channels_reduced.min(), f_channels_reduced.max()],
                            vmin=np.nanpercentile(waterfall_reduced, 5), vmax=np.nanpercentile(waterfall_reduced, 95))
            #ax1.set_ylabel('Flux (mJy)',fontsize=45)
            ax1.legend(loc = 'upper right',fontsize=42)
            ax2.set_xlabel('Time (seconds)',fontsize=45)
            #ax2.set_ylabel('Frequency (MHz)',fontsize=45)
            ax2.set_xticks([0.45,0.5,0.55])
            #ax2.set_xticks([])
